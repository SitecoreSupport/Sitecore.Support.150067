define([
    ...
    this.toggleRecipientsActionControl = function(recipientListType) {
        ...
        var recipientListsActionControl,
            recipientListControl,
            removeAction,
            editAction,
            setDefaultAction,
            removeBouncedRecipientsAction,
            removeUnsbuscribedAction,
            removeComplainedRecipientsAction;
        if (recipientListType === "{AAD5DC30-CC86-4988-BAF1-98661B02B79B}") {
        ...
        var isReadOnly = contextApp.MessageContext.get("isReadonly"),
            ...
            isSent = contextApp.MessageContext.get("messageState") === Constants.MessageStates.SENT;
        ...
        if ((recipientListSelectedItem && recipientListSelectedItem !== "" && recipientListControl.get("checkedItems").length === 0)
            ...
            this.enableAction(recipientListsActionControl, setDefaultAction, type !== "Segmented list" && !isDefault && !isReadOnly);
    ...
    this.addListToListControl = function(messageId, recipientListId, recipientListType) {
        if (typeof recipientListId != "undefined" && recipientListId != null) {
            ServerRequest(Constants.ServerRequests.ADD_RECIPIENT_LIST, {
                data: { messageId: messageId, recipientListId: recipientListId, type: recipientListType },
        ...
        if (typeof recipientListId != "undefined" && recipientListId != null) {
            ...
            for (var index = 0; index < items.length; ++index) {
                if (items[index].default) {
                    items[index].defaultText = contextApp.RecipientsStringDictionary.translate("ECM.Pages.Recipients.Yes");
    ...
    this.removeList = function(messageId, recipientListType) {
        ...
        ServerRequest(Constants.ServerRequests.REVOME_RECIPIENT_LIST, {
            data: { messageId: messageId, recipientListIds: itemIds, type: recipientListType },
            success: function(response) {
                if (response.error) {
                    return;
                }
                ...
                for (index = 0; index < oldItems.length; ++index) {
                    if (!_.contains(itemIds, oldItems[index].itemId)) {
                        ...
                        isRemovedListWasDefault = oldItems[index].default;
                    }
                }
                ...
                if (defaultListIndex > 0) {
                    ...
                    defaultList.default = true;
                    defaultList.defaultText = contextApp.RecipientsStringDictionary.translate("ECM.Pages.Recipients.Yes");
                    newItems.splice(defaultListIndex, 1);
                    newItems.splice(0, 0, defaultList);
                } else if (defaultListIndex === 0) {
                    newItems[defaultListIndex].default = true;
                    newItems[defaultListIndex].defaultText = contextApp.RecipientsStringDictionary.translate("ECM.Pages.Recipients.Yes");
                }
    ...
    this.setDefaultList = function(messageId, recipientListType) {
        ...
        ServerRequest(Constants.ServerRequests.SET_DEFAULT_RECIPIENT_LIST, {
            data: { messageId: messageId, recipientListId: recipientListId, type: recipientListType },
            success: function(response) {
                if (response.error) {
                    return;
                ...
                if (defaultListIndex > 0) {
                    ...
                    items[defaultListIndex].default = true;

                    items[0].defaultText = "";
                    items[defaultListIndex].defaultText = contextApp.RecipientsStringDictionary.translate("ECM.Pages.Recipients.Yes");
    ...
    return {
        init: function(app) {
            ...
            contextApp.on("add:included:from:existinglist", _.bind(this.addListByType, this, 'IncludedRecipient'));
            ...
            contextApp.on("add:excluded:from:existinglist", _.bind(this.addListByType, this, 'ExcludedRecipient'));
            ...
            // TODO: check is this method deprecated.
        ...
        addListByType: function (listType) {
            var listDataSource = contextApp[listType + "DataSource"];

            if (listDataSource) {
                self.addList(
                    listDataSource.get("messageId"),
                    listDataSource.get("recipientListType"),
                    function (callback, allExcludeItemsIds) {
                        DialogService.get('selectList').done(function (dialog) {
                            dialog.showDialog({
                                callback: function (selectedItemId, selectedItem) {
                                    if (callback) {
        ...
        addEmptyIncludedList: function() {
            DialogService.show('addEmptyList', {
                on: {
                    ok: self.addListToListControl
                },
                data: {
                    messageId: contextApp.IncludedRecipientDataSource.get("messageId"),
                    recipientListType: contextApp.IncludedRecipientDataSource.get("recipientListType")
                },
                notify: false
            });
        }
